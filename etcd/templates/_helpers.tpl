...
{{- $peerCertsOption := " --peer-cert=\"ETCD_PEER_CERT_FILE\" --peer-key=\"ETCD_PEER_KEY_FILE\"" -}}
{{- $peerCaOption := " --peer-trusted-ca-file=\"$ETCD_PEER_TRUSTED_CA_FILE\"" -}}
{{- $certsOption := " --cert=\"ETCD_CERT_FILE\" --key=\"ETCD_KEY_FILE\"" -}}
{{- $caOption := " --trusted-ca-file=\"$ETCD_TRUSTED_CA_FILE\"" -}}
{{- if .Values.auth.peer.secureTransport -}}
{{- printf "%s" $peerCertsOption -}}
{{- end -}}
{{- if .Values.auth.peer.enableAuthentication -}}
{{- printf "%s" $peerCaOption -}}
{{- end -}}
{{- if .Values.auth.client.secureTransport -}}
